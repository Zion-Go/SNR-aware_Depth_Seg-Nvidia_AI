...
test_path_left = '/workspaces/isaac_ros-dev/src/Semantic-Aware-Low-Light-Image-Enhancement/experiments_dataset/29_Aug_snr/SNR_dataset/left'
test_path_right = '/workspaces/isaac_ros-dev/src/Semantic-Aware-Low-Light-Image-Enhancement/experiments_dataset/29_Aug_snr/SNR_dataset/right'
output_path = '/workspaces/isaac_ros-dev/src/Semantic-Aware-Low-Light-Image-Enhancement/experiments_dataset/tables_figures/snr_unimatch_depth'

count = 0
jpg_list_left = sorted(glob.glob(os.path.join(test_path_left, '*.png')))
jpg_list_right = sorted(glob.glob(os.path.join(test_path_right, '*.png')))
...
for left_image, right_image in zip(jpg_list_left, jpg_list_right):
    ...
    left_image = torch.from_numpy(left_image).to(device) # .cuda() 
    right_image = torch.from_numpy(right_image).to(device) # .cuda()
    # print(type(self.left_image_raw), device)
    
    img_depth = inference(left_image, right_image)
    ##########################
    ...
    threeD = threeD  / 16
    x1, y1 = 200, 300
    distance1 = math.sqrt(threeD[x1-5][y1+5][0]**2 + threeD[x1-5][y1+5][1]**2 + threeD[x1-5][y1+5][2]**2)
    distance2 = math.sqrt(threeD[x1-5][y1-5][0]**2 + threeD[x1-5][y1-5][1]**2 + threeD[x1-5][y1-5][2]**2)
    distance3 = math.sqrt(threeD[x1+5][y1+5][0]**2 + threeD[x1+5][y1+5][1]**2 + threeD[x1+5][y1+5][2]**2)
    distance4 = math.sqrt(threeD[x1-6][y1+6][0]**2 + threeD[x1-6][y1+6][1]**2 + threeD[x1-6][y1+6][2]**2)
    distance5 = math.sqrt(threeD[x1-6][y1-6][0]**2 + threeD[x1-6][y1-6][1]**2 + threeD[x1-6][y1-6][2]**2)
    distance6 = math.sqrt(threeD[x1+6][y1+6][0]**2 + threeD[x1+6][y1+6][1]**2 + threeD[x1+6][y1+6][2]**2)
    distance7 = math.sqrt(threeD[x1-4][y1+4][0]**2 + threeD[x1-4][y1+4][1]**2 + threeD[x1-4][y1+4][2]**2)
    distance8 = math.sqrt(threeD[x1-4][y1-4][0]**2 + threeD[x1-4][y1-4][1]**2 + threeD[x1-4][y1-4][2]**2)
    distance9 = math.sqrt(threeD[x1+4][y1+4][0]**2 + threeD[x1+4][y1+4][1]**2 + threeD[x1+4][y1+4][2]**2)
    distance = (distance1 + distance2 + distance3 + distance4 + distance5 + distance6 + distance7 + distance8 + distance9) / 9 / 1000
    img_depth = cv2.rectangle(img_depth, (260,340), (340,260), color=(255,255,255), thickness=2)
    img_depth = cv2.putText(img_depth, "Depth: %.2fm" % (distance), (160, 200), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255,255,255), thickness=2)
    ##############################

    cv2.imwrite(f'{output_path}/unimatch_snr_{count}.jpg', img_depth)
    print(f'saved {count}')
    count += 1
