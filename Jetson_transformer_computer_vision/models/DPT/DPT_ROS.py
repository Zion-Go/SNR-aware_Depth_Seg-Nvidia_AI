    ...
    def dpt_pub_callback(self):
        ...
        for n in range(len(image_list) + 1):
            ...
            with torch.no_grad():
                ...
                cv2.imshow("Seg", img_dpt)
                key = cv2.waitKey(1)

                # save the outputs
                path = f"/workspaces/isaac_ros-dev/src/isaac_ros_common/ros2bag/img_dpt_{self.i}.jpg"
                # leftpath = path + f"/left/"+"left_" + {self.i} + ".jpg"
                # rightpath=path + f"/right/"+ "right_" + {self.i} + ".jpg"
                if key == ord("s"):
        
                    cv2.imwrite(path, img_dpt)
                    print("img_dpt saved to: " + path)
