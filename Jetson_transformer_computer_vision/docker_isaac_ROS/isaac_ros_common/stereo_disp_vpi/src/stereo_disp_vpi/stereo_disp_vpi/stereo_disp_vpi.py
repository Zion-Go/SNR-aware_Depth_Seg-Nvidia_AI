...
class Stereo_Depth(Node):
    ...
    def sync_raw_callback(self, imgl_raw_suber, imgr_raw_suber):
        ...
        cv2.imshow('left', self.left_raw)
        # cv2.imshow('right', self.right_raw)

        key = cv2.waitKey(1)
        raw_path = f"/workspaces/isaac_ros-dev/src/isaac_ros_common/ros2bag/raw_{self.i}.jpg"   
        if key == ord("s"):
            cv2.imwrite(raw_path, self.left_raw)
            # cv2.imwrite(raw_path, self.right_raw)
            print("left_raw saved to: " + raw_path)
    ...
    def disp_pub_callback(self):
        ...
        for self.left_image_raw, self.right_image_raw in zip(left_raw_list, right_raw_list):
            ...
            # self.disp = cv2.putText(self.disp, "RunTime = %.3fs/frame" % (runtime), org=(0, 20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.6, color=(0,0,0), thickness=2)
