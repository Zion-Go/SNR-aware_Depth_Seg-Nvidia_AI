...
class SNR_SKF(Node):
    def __init__(self):
        ...
        self.resize_sub = self.create_subscription(Image, 'right/image_resize', self.resize_sub_callback, qos_profile=qos_policy)
        self.snr_publish = self.create_publisher(Image, 'right/image_snr', 200)
        self.timer = self.create_timer(0.1, self.snr_pub_callback)
    ...
    def snr_pub_callback(self):
        ...
        for n in range(len(resize_image_list)):
            if n % 2:
                ...
                save_imgs = False
                if save_imgs:
                    path = f"/workspaces/isaac_ros-dev/src/isaac_ros_common/ros2bag/img_snr_{self.i}_2_Sep_2.jpg"
                    cv2.imwrite(path, rlt_img)
                    print("img_snr saved to: " + path)
                
                torch.cuda.empty_cache()
                # ROS msgs
                snr_msg = self.bridge.cv2_to_imgmsg(np.array(rlt_img), 'bgr8')
                ros_time = rclpy.clock.Clock().now().to_msg()
                snr_msg.header.frame_id = "right_camera"
