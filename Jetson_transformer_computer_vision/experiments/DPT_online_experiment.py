...
class DPT(Node):
    def __init__(self):
        ...
        self.dpt_sub = self.create_subscription(Image, 'right/image_resize', self.raw_sub_callback, qos_profile=qos_policy)
        self.dpt_publish = self.create_publisher(Image, 'right/image_dpt', 20)
        self.timer = self.create_timer(0.1, self.dpt_pub_callback)
    ...
    def raw_sub_callback(self, raw_msg):
        ...
        raw_path = f"/workspaces/isaac_ros-dev/Semantic-Aware-Low-Light-Image-Enhancement/experiments_dataset/tables_figures/raw_{self.i}.jpg"
    ...
    def dpt_pub_callback(self):
        ...
        for n in range(len(image_list) + 1):
            ...
            with torch.no_grad():
                ...
                # img_dpt = cv2.putText(img_dpt, "FPS = %.3f" % (fps), org=(0, 40), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.6, color=(255, 255, 255), thickness=1)
                ...
                # if key == ord("s"):
                dpt_path = f"/workspaces/isaac_ros-dev/Semantic-Aware-Low-Light-Image-Enhancement/experiments_dataset/tables_figures/dpt_{self.i}.jpg"
